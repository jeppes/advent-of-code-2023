...
let read_file name = 
  In_channel.read_lines name ~fix_win_eol:true
...
let list_to_pairs list = 
  let rec list_to_pairs_aux list acc =  match list with
    | a::b::rest -> list_to_pairs_aux rest ((a, b)::acc)
    | [] -> acc
    | _ -> failwith "invalid pair list" in
